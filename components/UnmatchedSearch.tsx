...
type Props = {
  records: VictimWithDebug[];
};

const PAGE_SIZE = 100;

export default function UnmatchedSearch({ records }: Props) {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((person) => {
      return (
        ...
        String(person._debug_location ?? "").toLowerCase().includes(q) ||
  ...
  return (
    <>
      <div className="mb-8">
        <input
          type="text"
          placeholder="Search unmatched records by name, age, location, date, or profession..."
          value={query}
          onChange={(e) => handleQueryChange(e.target.value)}
          className="w-full rounded-3xl border border-white/10 bg-white/5 px-5 py-4 text-white placeholder:text-zinc-500 outline-none shadow-[0_0_30px_rgba(255,255,255,0.02)] backdrop-blur-xl transition focus:border-amber-300/30 focus:bg-white/10"
        />
      </div>

      <div className="mb-6 h-px w-full bg-gradient-to-r from-transparent via-amber-300/20 to-transparent" />
