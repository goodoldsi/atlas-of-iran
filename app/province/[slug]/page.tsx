...
import ProvinceVictimSearch from "@/components/ProvinceVictimSearch";
...
export default async function ProvincePage({ params }: PageProps) {
  ...
  const records = allVictims.filter((person) => {
    if (!person.province) return false;
    return normalizeProvinceToSlug(person.province) === slug;
  });
  ...
  const provinceTitle = records[0].province ?? slugToTitle(slug);
          ...
          <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <p className="mt-4 max-w-2xl text-base leading-8 text-zinc-400 md:text-lg">
                Memorial entries currently associated with {provinceTitle}.
              </p>
            </div>
            ...
            <div className="w-full max-w-xs rounded-3xl border border-white/10 bg-white/5 p-6 shadow-[0_0_50px_rgba(255,255,255,0.03)] backdrop-blur-xl">
              ...
              <p className="mt-3 text-sm leading-6 text-zinc-400">
                Search across all province records.
              </p>
            </div>
          </div>
        </div>

        <ProvinceVictimSearch records={records} />
